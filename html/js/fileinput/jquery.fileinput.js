  ...
  var fileinput = function(element, options) {
    ...
    var initInput = function() {
      ...
      if ($element.attr('value')) {
        ...
        // show thumb
        if (isImage($element.attr('value')) && !$element.attr('multiple')) {
      ...
      template.find('.image-preview-input input:file').on('change', function(e) {
        ...
        if (typeof file !== 'undefined') {
          ...
          reader.onload = function (e) {
            ...
            if (files.length > 1) {
              prev.find('.image-preview-filename').val(files.length + ' Files Selected');
            }

            if (files.length == 1 && isImage(file.name)) {
