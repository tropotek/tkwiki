<?php
/**
 * @author Michael Mifsud <info@tropotek.com>
 * @link http://www.tropotek.com/
 * @license Copyright 2007 Michael Mifsud
 * @package Db
 */

/**
 * A column map
 *
 * @package Db
 */
class Tk_Db_Map_StringEncrypt extends Tk_Db_Map_Interface
{
    
    /**
     * create a string
     *
     * @param string $propertyName 
     * @param array $columnNames (optional)
     * @return Tk_Db_Map_EncryptString
     */
    static function create($propertyName, $columnNames = array())
    {
        return new self($propertyName, $columnNames);
    }
    
    function getPropertyValue($row)
    {
        $name = $this->getColumnName();
        if (isset($row[$name])) {
            return Tk_Util_Encrypt::decrypt($row[$name]);
        }
    }
    
    function getColumnValue($obj)
    {
        $name = $this->getPropertyName();
        if (isset($obj->$name)) {
            $value = $obj->$name;
            //$value = str_replace("\r\n", "\n", utf8_encode($obj->$name));
            $value = Tk_Db_MyDao::escapeString($value);
            return array($this->getColumnName() => enquote(Tk_Util_Encrypt::encrypt($value)));
        }
        return array($this->getColumnName() => enquote(''));
    }
    
}
